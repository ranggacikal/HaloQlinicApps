...
public class DikemasAdapter extends RecyclerView.Adapter<DikemasAdapter.DikemasViewHolder> {

    Context context;
    List<DataItem> dataDikemas;

    public DikemasAdapter(Context context, List<DataItem> dataDikemas) {
        this.context = context;
        this.dataDikemas = dataDikemas;
    }

    @NonNull
    @NotNull
    @Override
    public DikemasViewHolder onCreateViewHolder(@NonNull @NotNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_history, parent, false);
        return new DikemasViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull @NotNull DikemasViewHolder holder, int position) {

        String urlImage = "";
        String nama = "";
        int harga = 0;

        List<ProdukItem> produkItems = dataDikemas.get(position).getProduk();
        ...
        holder.txtIdHistory.setText(dataDikemas.get(position).getIdTransaksi());
        holder.txtStatus.setText(dataDikemas.get(position).getStatusPesanan());

        holder.itemView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(context, DetailTransaksiActivity.class);
                intent.putExtra("id_transaksi", dataDikemas.get(position).getIdTransaksi());
                intent.putExtra("id_member", dataDikemas.get(position).getIdMember());
    ...
    public int getItemCount() {
        return dataDikemas.size();
    }

    public class DikemasViewHolder extends RecyclerView.ViewHolder {

        ImageView imgHistory;
        TextView txtIdHistory, txtNamaProduk, txtHarga, txtStatus;
        public DikemasViewHolder(@NonNull @NotNull View itemView) {
            super(itemView);
