...
public class CariJadwalSpesialisAdapter extends RecyclerView.Adapter<CariJadwalSpesialisAdapter.CariJadwalSpesialisViewHolder> {

    Context context;
    List<DataItem> dataCari;

    public CariJadwalSpesialisAdapter(Context context, List<DataItem> dataCari) {
    ...
    @NotNull
    @Override
    public CariJadwalSpesialisViewHolder onCreateViewHolder(@NonNull @NotNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_dokter, parent, false);
        return new CariJadwalSpesialisViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull @NotNull CariJadwalSpesialisViewHolder holder, int position) {
    ...
    public class CariJadwalSpesialisViewHolder extends RecyclerView.ViewHolder {

        ImageView imgDokter;
        TextView txtNama, txtSpesialis, txtPengalaman, txtHarga;
        Button btnChat, btnBuatJadwal;

        public CariJadwalSpesialisViewHolder(@NonNull @NotNull View itemView) {
            super(itemView);
